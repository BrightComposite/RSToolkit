...
namespace asd
{
	...
	{
	public:
		api(gui) Text(asd::graphics * graphics, const WideString & s, const handle<Font> & font, const Color & color = Color::black, int fontSize = 14);
		virtual ~Text() {}
		...
		virtual api(gui) void draw(const int_point & pos);
		...
		static api(gui) void setDynamic(widget * w, const WideString & s, const handle<Font> & font, const Color & color = Color::black, int fontSize = 14);
		static api(gui) void setSmart(widget * w, const WideString & s, const handle<Font> & font, const Color & color = Color::black, int fontSize = 14);
		static api(gui) void setContents(widget * w, const WideString & s);
	...
	public:
		api(gui) TextLayer(widget * w, unique<Text> && text);
	...
	protected:
		virtual api(gui) void draw(widget * w) override;
		virtual void update() {}
	...
	create_layer_component(gui, TextLayer);

	class StaticTextLayer : public TextLayer
	{
	public:
		api(gui) StaticTextLayer(widget * w, unique<Text> && text);
	...
	protected:
		virtual api(gui) void draw(widget * w) override;
		virtual api(gui) void update() override;
	...
	protected:
		...
		static api(gui) void dynamicDraw(SmartTextLayer * layer, widget * w);
		static api(gui) void staticDraw(SmartTextLayer * layer, widget * w);
