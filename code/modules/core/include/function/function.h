 ...
 *
 * 	Changes:
 * 		Changed namespace from `func` to `asd`
 * 		Added guard block
 *
...
#		endif
		...
		struct type_defer<Result, Argument>
		{
			typedef Result result_type;
			typedef Argument argument_type;
		};
		template <typename Result, typename First_Argument, typename Second_Argument>
		struct type_defer<Result, First_Argument, Second_Argument>
		{
			typedef Result result_type;
	...
	class function<Result(Arguments...)>
		: public detail::type_defer<Result, Arguments...>
	{
	public:
