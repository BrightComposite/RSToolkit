...
namespace asd
{
	...
	template<>
	struct Cast<vector, btVector3>
	{
		static inline void cast(btVector3 & out, const vector & in) {
			out.setValue(in.x, in.y, in.z);
		}
	};
	
	template<>
	struct Cast<quaternion, btQuaternion>
	{
		static inline void cast(btQuaternion & out, const quaternion & in) {
			out.setValue(in.x, in.y, in.z);
		}
	};
