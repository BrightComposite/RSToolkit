...
namespace Rapture
{
	...
	{
		static const unsigned long crc32_table[256] = /* CRC-32 Lookup Table */
		{
...
#define CrcIterator(i) if(i == size)break; crc = ((crc >> 8) & 0xFFFFFFFFUL) ^ crc32_table[(crc ^ buf[i]) & 0xFFUL];

#define CrcIterator0 CrcIterator(__COUNTER__)
#define CrcIterator1 CrcIterator0 CrcIterator0
#define CrcIterator2 CrcIterator1 CrcIterator1
#define CrcIterator3 CrcIterator2 CrcIterator2
#define CrcIterator4 CrcIterator3 CrcIterator3
#define CrcIterator5 CrcIterator4 CrcIterator4
#define CrcIterator6 CrcIterator5 CrcIterator5
#define CrcIterator7 CrcIterator6 CrcIterator6
#define CrcIterator8 CrcIterator7 CrcIterator7

		while(true)
		{
			CrcIterator8 // 256 iterations
			break;
		}
	...
	inline unsigned long crc32(const void * buffer, size_t size)
	{
		return crc32(buffer, size, 0xFFFFFFFFUL);
	}

	template <size_t N>
	inline unsigned long crc32(const char(&s)[N])
	{
		return crc32(s, N - 1, 0xFFFFFFFFUL);
	}
}
