...
namespace asd
{
    namespace management
    {
        ...
        template <typename T>
        auto copy(const void *, void *) -> std::enable_if_t<!std::is_copy_constructible<T>::value> {
        ...
        void destroy(InputIterator first, InputIterator last, std::int8_t * data) {
            std::for_each(first, last, [data](const auto & object) {
                destroy(object, data);
            });
        }
    ...
    {
    private:
        using object_info_container_type = asd::array_list<management::object_info_t>;
    ...
    public:
        template <typename Head, typename ... Tail, useif<sizeof...(Tail) != 0 || !std::is_same<std::decay_t<Head>, any_list>::value>>
        ...
        void push_back(T && object) {
            auto creation_place = set_up_creation_place<T>();
            accomodate(creation_place, std::forward<T>(object));
        }

        template <typename T, typename ... A>
        void emplace_back(A && ... args) {
            auto creation_place = set_up_creation_place<T>();
            construct<T>(creation_place, std::forward<A>(args)...);
        }
    ...
    private:
        ...
        template <typename T>
        std::int8_t * set_up_creation_place() {
            if(auto proposed_creation_place = try_to_align<T>()) {
                return proposed_creation_place;
            } else {
                reserve(volume() + sizeof(T) + alignof(T));
                return force_align<T>();
            }
        }
        
        template <typename T>
        void push_back_no_realloc(T && object) {
            auto creation_place = force_align<T>();
            accomodate(std::forward<T>(object), creation_place);
        }
        
        template <typename T>
        std::int8_t * try_to_align() {
        ...
        template <typename T>
        void accomodate(std::int8_t * creation_place, T && object) {
        ...
        template <typename T, typename ... A>
        void construct(std::int8_t * creation_place, A && ... args) {
            using raw_type = std::decay_t<T>;
            new(creation_place) raw_type(std::forward<A>(args)...);
