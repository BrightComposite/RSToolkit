...
namespace asd
{
	...
	public:
...
#else
		api(ui)
		UISpace(Graphics * graphics, const IntSize & size);
#endif
		...
		api(ui)
		const IntPoint & cursorPos() const;
		api(ui)
		void setCursorPos(const IntPoint & pt);
		api(ui)
		void mouseUpdate();
		...
		api(ui)
		void clipCursor(const IntRect & region);
		api(ui)
		void unclipCursor();
	...
	protected:
		...
		auto & pool() {
			static asd::map<::Window, UISpace *> _pool;
			return _pool;
		}
#endif
