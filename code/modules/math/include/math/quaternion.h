...
namespace asd
{
    namespace math
    {
        ...
        struct alignas(sizeof(T) * 4) quaternion : aligned_alloc
        {
            ...
            using VectorConstants = vector_constants<T>;
            ...
            quaternion() : v(VectorConstants::positive_w) {}
            ...
            quaternion(const VectorType & axis, T angle) : quaternion(trigon(angle * 0.5f).template shuffle<0, 0, 0, 1>() * axis.template blend<0, 0, 0, 1>(VectorConstants::positive_w)) {}
            ...
            VectorType right() const {
                return apply_to(VectorConstants::right);
            }
            
            VectorType up() const {
                return apply_to(VectorConstants::up);
            }
            
            VectorType forward() const {
                return apply_to(VectorConstants::forward);
            }
            ...
            quaternion & rotate_x(T angle) {
                return rotate(VectorConstants::positive_x, angle);
            }
            
            quaternion & rotate_y(T angle) {
                return rotate(VectorConstants::positive_y, angle);
            }
            
            quaternion & rotate_z(T angle) {
                return rotate(VectorConstants::positive_z, angle);
            }
            ...
            static quaternion from_euler(const VectorType & angles) {
                auto halfs = angles * VectorConstants::half;
            ...
            static quaternion from_vectors(const VectorType & from, const VectorType & to) {
                ...
                if(d.x < (-1.0f + Constants::eps)) { // 180 degrees, find any acceptable orthogonal vector to represent a rotation
                    auto axis = cross(VectorConstants::positive_x, from);
                    
                    if(axis.magnitudeSq() < Constants::eps2) {
                        axis = cross(VectorConstants::positive_y, from);
                    }
                    
                    return {axis.normalize(), Constants::pi};
                }
                
                auto v = cross(from, to).template blend<0, 0, 0, 1>(VectorConstants::one + d); // orthogonal vector [x y z] and rotation angle [w]
            ...
            matrix <T> to_matrix() const {
                ...
                return {
                    VectorConstants::positive_x + VectorConstants::two_xyz * s.template blend<0, 1, 1, 0>((a + b.template negate<0, 0, 1, 0>()).template shuffle<2, 2, 1, 1>()),    // 1.0f - 2.0f * (y * y + z * z),	2.0f * (x * y - w * z),			2.0f * (x * z + w * y),			0.0f
                    VectorConstants::positive_y + VectorConstants::two_xyz * s.template blend<1, 0, 1, 0>((a + b).template shuffle<2, 2, 0, 0>()),                                    // 2.0f * (x * y + w * z),			1.0f - 2.0f * (x * x + z * z),	2.0f * (y * z - w * x),			0.0f
                    VectorConstants::positive_z + VectorConstants::two_xyz * (a - b).template shuffle<1, 0, 2, 3>(s),                                                                // 2.0f * (x * z - w * y),			2.0f * (y * z + w * x),			1.0f - 2.0f * (x * x + y * y),	0.0f
                    VectorConstants::positive_w
                };
            }
            ...
            void to_matrix(matrix <T> & m) const {
                ...
                m[0] = VectorConstants::positive_x + VectorConstants::two_xyz * s.template blend<0, 1, 1, 0>((a + b.template negate<0, 0, 1, 0>()).template shuffle<2, 2, 1, 1>()); // 1.0f - 2.0f * (y * y + z * z),	2.0f * (x * y - w * z),			2.0f * (x * z + w * y),			0.0f
                m[1] = VectorConstants::positive_y + VectorConstants::two_xyz * s.template blend<1, 0, 1, 0>((a + b).template shuffle<2, 2, 0, 0>());                                // 2.0f * (x * y + w * z),			1.0f - 2.0f * (x * x + z * z),	2.0f * (y * z - w * x),			0.0f
                m[2] = VectorConstants::positive_z + VectorConstants::two_xyz * (a - b).template shuffle<1, 0, 2, 3>(s);                                                            // 2.0f * (x * z - w * y),			2.0f * (y * z + w * x),			1.0f - 2.0f * (x * x + y * y),	0.0f
                m[3] = VectorConstants::positive_w;
            }
