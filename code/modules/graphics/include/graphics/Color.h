...
namespace asd
{
	...
	struct ColorTraits<ColorType::Byte>
	{
		...
		static inline const math::byte_vector & default_color() {
			static const math::byte_vector v{0xFF, 0xFF, 0xFF, 0xFF};
			return v;
		}
	...
	struct ColorTraits<ColorType::Float>
	{
		...
		static inline const math::float_vector & default_color() {
			return math::float_vector::one;
		}
	...
	template<ColorType T>
	using ColorVector = math::vector<color_data_t<T>>;
	
	template<ColorType datatype, ColorFormat format>
	struct ColorBase {};
	
	template<ColorType datatype>
	struct ColorBase<datatype, ColorFormat::rgb> : aligned_alloc
	{
		...
		ColorBase(const math::vector <T> & vec) : data{vec.v} {}
		
		ColorBase(const math::vector <T> & vec, T alpha) : data{vec.x, vec.y, vec.z, alpha} {}
	...
	template<ColorType datatype>
	struct ColorBase<datatype, ColorFormat::hsv> : aligned_alloc
	{
		using T = color_data_t<datatype>;
		...
		ColorBase(ColorBase && color) : data{std::move(color.data)} {}
		...
		ColorBase(array<T, 4> && data) : data{std::forward<array<T, 4>>(data)} {}
	...
	struct GenericColor : ColorBase<datatype, format>
	{
		...
		GenericColor(const math::vector <T> & color) : Base{color} {}
		
		GenericColor(const math::vector <T> & color, T alpha) : Base{color, alpha} {}
		...
		math::vector <T> vector() const {
			return {this->m};
		}
		
		void vector(const math::vector <T> & v) {
			this->data = v.v;
		}
	...
	struct ColorDataCast<ColorType::Byte, ColorType::Float>
	{
		...
		static inline void cast(GenericColor<ColorType::Float, format> & out, const GenericColor<ColorType::Byte, format> & in) {
			out.vector(math::float_vector(in.vector()) / 255.0f);
		}
	};
	...
	struct ColorDataCast<ColorType::Float, ColorType::Byte>
	{
		...
		static inline void cast(GenericColor<ColorType::Byte, format> & out, const GenericColor<ColorType::Float, format> & in) {
			out.vector(math::byte_vector(in.vector() * 255.0f));
		}
	};
	...
	struct ColorFormatCast<ColorFormat::rgb, ColorFormat::hsv>
	{
		...
		static inline void cast(GenericColor<ColorType::Float, ColorFormat::hsv> & out, const GenericColor<ColorType::Float, ColorFormat::rgb> & color) {
			...
			if(delta < math::constants<float>::eps) {
				return;
			}
	...
	template<ColorType datatype, ColorFormat format>
	template<ColorType d, ColorFormat f, used_t>
	GenericColor<datatype, format>::GenericColor(const GenericColor<d, f> & color) {
		cast::to(*this, color);
	}
}
